"""
Módulo de integração entre o OrquestradorAdapter e o WondercomClientPlaywright.
Este arquivo implementa a adaptação necessária para utilizar o cliente Playwright.
"""
...
try:
    from vm_api.wondercom_client_playwright import WondercomClient
    logger.info("Módulo WondercomClient (Playwright) importado com sucesso")
except ImportError as e:
    logger.error(f"Erro ao importar WondercomClient (Playwright): {e}")
    raise
...
class WondercomIntegration:
    ...
    DEFAULT_CHROME_DRIVER_PATH = "/usr/local/bin/chromedriver"  # Mantido para compatibilidade
    ...
    def alocar_wo(work_order_id, credentials, chrome_driver_path=None, portal_url=None):
        ...
        Args:
            ...
            chrome_driver_path (str, opcional): Caminho para o chromedriver (ignorado, mantido para compatibilidade)
        ...
        try:
            ...
            client = WondercomClient(
                chrome_driver_path=chrome_driver_path,  # Ignorado pelo Playwright, mantido para compatibilidade

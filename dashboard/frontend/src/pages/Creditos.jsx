...
const Creditos = () => {
  ...
  return (
    <div className="p-4 md:p-6">
      <h1 className="text-xl font-semibold text-text mb-4">Meus Créditos</h1>
      
      {/* Card de créditos */}
      <div className="bg-card rounded-xl shadow p-6 flex items-center justify-between mb-6">
      ...
      <button 
        className="w-full bg-primary text-card py-3 rounded-xl font-semibold shadow hover:bg-primary-dark transition flex items-center justify-center gap-2"
      ...
      {showRechargeModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-card rounded-xl p-6 w-full max-w-md">
            <h2 className="text-xl font-semibold text-text mb-4">Recarregar Créditos</h2>
            
            <div className="mb-6">
              <label className="block text-sm text-muted mb-2">Quantidade de créditos</label>
              <div className="flex items-center">
                <button 
                  className="bg-gray-100 px-3 py-2 rounded-l-lg text-text"
                ...
                />
                <button 
                  className="bg-gray-100 px-3 py-2 rounded-r-lg text-text"
                  onClick={() => setRechargeAmount(rechargeAmount + 5)}
                >
                  +
                </button>
              </div>
              <p className="text-sm text-muted mt-2">Valor: €{(rechargeAmount * 0.50).toFixed(2)}</p>
            </div>
            
            <div className="mb-6">
              <label className="block text-sm text-muted mb-2">Método de pagamento</label>
              <div className="border border-gray-200 rounded-lg p-3 flex items-center gap-3">
                <CreditCard size={20} className="text-muted" />
                <div>
                  <p className="text-text">Cartão de crédito</p>
            ...
            <div className="flex gap-3">
              <button 
                className="flex-1 border border-gray-200 py-2 rounded-lg text-muted hover:bg-gray-50 transition"
                onClick={() => setShowRechargeModal(false)}
              >
                Cancelar
              </button>
              <button 
                className="flex-1 bg-primary text-card py-2 rounded-lg font-medium hover:bg-primary-dark transition"

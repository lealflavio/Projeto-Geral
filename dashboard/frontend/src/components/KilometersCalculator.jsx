                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input
                                type="date"
                                id="startDate"
                                className="w-full rounded-md border border-gray-300 py-2 pl-3 pr-10 text-gray-700 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                                value={startDate}
                ...
                <div className="mb-4">
                    ...
                    <div className="relative">
                        <input
                            ...
                            disabled={loading}
                        />
                        <Home size={18} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400" />
                    </div>
                ...
                <div className="bg-white rounded-lg p-6 border border-gray-200">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors flex items-center gap-2"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div className="bg-green-50 p-4 rounded-lg border border-green-100">
                            <p className="text-sm text-green-600 mb-1">Valor a receber (€0,36/km)</p>
                            <p className="text-2xl font-bold text-green-700">{formatCurrency(result.total_km * 0.36)}</p>
                        </div>
                    </div>
                    
                    <h3 className="text-md font-medium text-[#333] mb-3">Detalhes do Percurso</h3>
                    
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-full">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                    <th className="py-3 px-4 text-left text-xs font-medium text-[#555] uppercase tracking-wider">Origem</th>
                                    <th className="py-3 px-4 text-left text-xs font-medium text-[#555] uppercase tracking-wider">Destino</th>
                                    <th className="py-3 px-4 text-right text-xs font-medium text-[#555] uppercase tracking-wider">Distância</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100">
                                {result.detalhes.map((item, index) => (
                                    <tr key={index} className="hover:bg-gray-50 transition-colors">
                                        <td className="py-3 px-4 text-sm text-[#555]">{item.origem}</td>
                                        <td className="py-3 px-4 text-sm text-[#555]">{item.destino}</td>
                                        <td className="py-3 px-4 text-sm font-medium text-[#333] text-right">{item.distancia.toFixed(1)} km</td>
                                    </tr>
                                ))}
                                <tr className="bg-gray-50">
                                    <td className="py-3 px-4 text-sm font-medium text-[#333]" colSpan="2">Total</td>
                                    <td className="py-3 px-4 text-sm font-bold text-blue-600 text-right">{result.total_km.toFixed(1)} km</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
